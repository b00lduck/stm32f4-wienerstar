...
{
        mat4x4_t res;
#ifdef USE_ARM_MATH_OPS
    	arm_matrix_instance_f32 src1, src2, dst;
    	src1.numCols = src1.numRows = src2.numCols = src2.numRows = dst.numCols = dst.numRows = 4;
    	src1.pData = a.m;
    	src2.pData = b.m;
    	dst.pData = res.m;
...
void mat4x4mul3(mat4x4_t* dst, mat4x4_t* a, mat4x4_t* b)
{
#ifdef USE_ARM_MATH_OPS
    	arm_matrix_instance_f32 src1, src2, dest;
    	src1.numCols = src1.numRows = src2.numCols = src2.numRows = dest.numCols = dest.numRows = 4;
    	src1.pData = a->m;
    	src2.pData = b->m;
    	dest.pData = dst->m;
    	arm_mat_mult_f32(&src1, &src2, &dst);
