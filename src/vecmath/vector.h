...
/**
 * TODO
 * - check for "real" FPU vsqrt.32 usage
 *
 */
...
static inline vec2_t vec2cyl(float r,float angle)
{
	...
	arm_sin_cos_f32(angle, &angle_sin, &angle_cos);
	return vec2(r*angle_cos, r*angle_sin);
#else
...
static inline vec2_t vec2rot(vec2_t v,float angle)
{
	...
	arm_sin_cos_f32(angle, &angle_sin, &angle_cos);
	return vec2(
		v.x*angle_cos-v.y*angle_sin,
		v.x*angle_sin+v.y*angle_cos);
#else
	return vec2(
